<template>
  <div class="archive-exhibition-page">
    <article v-if="exhibition" class="exhibition-content" role="article">
      <header class="exhibition-header">
        ...
        <p
          ...
          class="exhibition-dates"
        >
          <time v-if="exhibition.date_start" :datetime="exhibition.date_start">
            {{ formatDate(exhibition.date_start) }}
          </time>
          <span v-if="exhibition.date_start && exhibition.date_end"> – </span>
          <time v-if="exhibition.date_end" :datetime="exhibition.date_end">
      ...
      <nav
        ...
        class="rooms-navigation"
        aria-label="Navigate between rooms"
      >
      ...
      <footer class="exhibition-footer">
        <NuxtLink to="/exhibitions/current" class="back-link">
          ← Current Exhibition
        </NuxtLink>
...
const { data: exhibition, error } = await useAsyncData(
  `archive-exhibition-${slug}`,
  () => queryContent(`/archive/${slug}`).findOne(),
)
...
useHead({
  title: `${exhibition.value?.title ?? 'Exhibition'} – Gibson Accessible Exhibition Didactics`,
  meta: [
    {
      name: 'description',
      content: exhibition.value?.description ?? `Archived exhibition`,
    },
  ],
})
